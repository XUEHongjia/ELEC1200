...
textbit = text2bitseq(textmsg);  % turning text to bits according to ASCII
textwave = bitseq2waveform(textbit,SPB);  % turning bits to wave

% preparing bits for protocal -> starts
training0 = zeros(1,200);
training1 = ones(1,200);
training2 = zeros(1,200);
trainingseq = [training0,training1,training2];
% preparing bits for protocal -> ends


textwave1 = [ones(1,SPB),trainingseq,textwave];  % turning protocal bits to waves  

rx_wave = txrx(textwave1); % transmit the protocal through the noisy channel

% sorting out the pramaters for protocal -> starts
...
% sorting out the pramaters for protocal -> ends
